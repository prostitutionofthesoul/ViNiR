        ...
        ColumnLayout {
            ...
            CollapsibleSection {
                ...
                GridLayout {
                    ...
                    ActionTile {
                        ...
                        onClicked: Quickshell.execDetached(["/usr/bin/dolphin"])
                    }
                    ActionTile {
                        tileIcon: "settings"
            ...
            CollapsibleSection {
                ...
                ActionButton {
                    ...
                    onClicked: Quickshell.execDetached(["/usr/bin/sh", "-c", "qs kill -c ii; qs -c ii -d"])
                }
                ActionButton {
                    btnIcon: "lock"
            ...
            CollapsibleSection {
                ...
                ActionButton {
                    ...
                    onClicked: Quickshell.execDetached([Config.options?.apps?.terminal ?? "/usr/bin/kitty", "-e", "sh", "-c", "sudo xbps-install -Syu; echo 'Done. Press Enter to close...'; read _"])
                }
                ActionButton {
                    btnIcon: "cleaning_services"
                    label: Translation.tr("Clean package cache")
                    onClicked: Quickshell.execDetached([Config.options?.apps?.terminal ?? "/usr/bin/kitty", "-e", "sh", "-c", "sudo xbps-remove -O; echo 'Cache cleaned. Press Enter to close...'; read _"])
                }
                ActionButton {
                    btnIcon: "info"
                    label: Translation.tr("System info")
                    onClicked: Quickshell.execDetached([Config.options?.apps?.terminal ?? "/usr/bin/kitty", "-e", "sh", "-c", "fastfetch; echo 'Press Enter to close...'; read _"])
                }
            }
