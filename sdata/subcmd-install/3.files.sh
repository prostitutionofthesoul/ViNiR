...
# Dolphin config — only if nautilus is installed (nautilus is now the default file manager)
if command -v nautilus &>/dev/null; then
  if [[ -f "defaults/kde/nautilusrc" ]]; then
    install_file "defaults/kde/nautilusrc" "${XDG_CONFIG_HOME}/nautilusrc"
  elif [[ -f "dots/.config/nautilusrc" ]]; then
    install_file "dots/.config/nautilusrc" "${XDG_CONFIG_HOME}/nautilusrc"
  fi
  ...
  # Dolphin stores panel visibility in nautilusstaterc which overrides nautilusrc.
  # Policy:
  ...
  # - User can force reset via --reset-nautilus-layout.
  DOLPHIN_STATE_FILE="${XDG_STATE_HOME:-$HOME/.local/state}/nautilusstaterc"
  if [[ "${INSTALL_FIRSTRUN}" == true || "${RESET_DOLPHIN_LAYOUT}" == true ]]; then
    if [[ -f "defaults/kde/nautilusstaterc" ]]; then
      mkdir -p "${XDG_STATE_HOME:-$HOME/.local/state}"
      install_file "defaults/kde/nautilusstaterc" "${DOLPHIN_STATE_FILE}"
...
# These files are no longer created - kdeglobals and nautilusrc are always overwritten
for obsolete_new in "${XDG_CONFIG_HOME}/kdeglobals.new" \
                    "${XDG_CONFIG_HOME}/nautilusrc.new"; do
...
FILE_MANAGER=""
for fm in org.kde.nautilus.desktop thunar.desktop pcmanfm.desktop org.kde.nautilus.desktop; do
