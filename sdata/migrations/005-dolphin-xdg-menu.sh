...
MIGRATION_ID="005-nautilus-xdg-menu"
...
Will add spawn-at-startup:
  spawn-at-startup "bash" "-c" "# systemctl --user (not needed on void) XDG_MENU_PREFIX && kbuildsycoca6"
DIFF
}
...
migration_apply() {
  ...
  if ! grep -q 'import-environment XDG_MENU_PREFIX' "$config"; then
    if grep -q 'spawn-at-startup' "$config"; then
      sed -i '0,/spawn-at-startup/s//spawn-at-startup "bash" "-c" "# systemctl --user (not needed on void) XDG_MENU_PREFIX \&\& kbuildsycoca6"\n\nspawn-at-startup/' "$config"
    fi
  fi
}
